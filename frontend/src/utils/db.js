...
class Database {
    ...
    checkMigration() {
        if (this.users.length > 0 && !this.users[0].email) {
            console.log("DB: Migrating email fields...");
            this.users = SEED_USERS;
            this.saveUsers();
        }
    }
    ...
    saveUsers() { localStorage.setItem(KEYS.USERS, JSON.stringify(this.users)); }
    saveProjects() { localStorage.setItem(KEYS.PROJECTS, JSON.stringify(this.projects)); }
    saveLogs() { 
        localStorage.setItem(KEYS.LOGS, JSON.stringify(this.logs));
        window.dispatchEvent(new Event("db_update_logs"));
    }
    saveQC() { localStorage.setItem(KEYS.QC, JSON.stringify(this.qcRecords)); }
    saveConfig() { localStorage.setItem(KEYS.CONFIG, JSON.stringify(this.incentiveConfig)); }
    saveRoster() { localStorage.setItem(KEYS.ROSTER, JSON.stringify(this.roster)); }
    saveGuidelines() { localStorage.setItem(KEYS.GUIDELINES, JSON.stringify(this.guidelines)); }
    saveRequests() { localStorage.setItem(KEYS.REQUESTS, JSON.stringify(this.passwordRequests)); }
